        	...
        	for (int i = 0; i < cards.size() - 1 && !found; i++) {
        		for (int j = i+1; j < cards.size() && !found; j++) {
        			for (int h = j+1; h < cards.size() && !found; h++) {
        				if (cards.get(i).getRank() == cards.get(j).getRank() && cards.get(j).getRank() == cards.get(h).getRank()) found = true;
        			}
        		}
        	}
...
//    	boolean found = false;
//    		for (int i = 0; i < cards.size() - 1 && !found; i++) {
//    			if (cards.get(i).getSuit() == cards.get(i)
//    		}
        return false;
    }
    	...
    	for (int i = 0; i < cards.size() - 1 && !found; i++) {
    		for (int j = i+1; j < cards.size() && !found; j++) {
    			for (int h = j+1; h < cards.size() && !found; h++) {
    				...
    				if (cards.get(i).getRank() == cards.get(j).getRank() && cards.get(j).getRank() == cards.get(h).getRank() && cards.get(h).getRank() == cards.get(k).getRank()) found = true;
    			}
    		}
    	}
    	}
